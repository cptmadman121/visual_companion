    ...
    private static async Task HandleAsync(HttpListenerContext ctx)
    {
        try
        {
            ...
            if (ctx.Request.HttpMethod == "POST" && ctx.Request.Url?.AbsolutePath == "/v1/process")
            {
                ...
                {
                    "proofread" => DeskLLM.Avalonia.Configuration.PromptShortcutConfiguration.DefaultProofreadPrompt,
                    "translate" => DeskLLM.Avalonia.Configuration.PromptShortcutConfiguration.DefaultTranslatePrompt,
                    _ => req.ExtraPrompt
                };
